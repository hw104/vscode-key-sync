...
export async function updateHandler(context: vscode.ExtensionContext, option: InnerOption = { force: false }): Promise<void> {
    ...
    if (!option.force) {
        ...
        if (changes.length !== 0) {
            vscode.window.showWarningMessage('Cannot Auto Update: keybindings.json has been changed since the last commit. To Force Update, run "Key Sync: Update (Force)"');
            return;
        }
    }
    ...
    vscode.window.showInformationMessage(`keybindings.json was updated from ${remote?.pushUrl}`);
}
